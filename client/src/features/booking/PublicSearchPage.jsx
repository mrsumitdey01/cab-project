...
export function PublicSearchPage() {
  ...
  const isCinematicHero = siteContent?.heroVariant === 'cinematic';
  ...
  return (
    <div className="w-full pb-10">
      <div
        className={`${isCinematicHero ? 'relative mb-[-80px] overflow-hidden rounded-b-[3rem] px-6 pb-36 pt-24 shadow-2xl md:min-h-[85vh] md:pt-32' : 'hero-premium-bg relative mb-[-80px] rounded-b-[3rem] px-6 pb-32 pt-20 shadow-2xl'} z-0`}
      >
        {isCinematicHero ? (
          <>
            <img
              src="https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=1800&q=80"
              alt="Luxury sedan on a highway"
              className="absolute inset-0 h-full w-full scale-110 object-cover object-[75%_center] md:scale-100 md:object-center"
            />
            <div className="absolute inset-0 bg-indigo-950/40" />
            <div className="absolute inset-x-0 bottom-0 h-[28%] bg-gradient-to-b from-transparent via-[#faf8ff]/50 to-[#faf8ff]" />
            <div className="absolute left-1/2 top-20 h-72 w-72 -translate-x-1/2 rounded-full bg-indigo-950/35 blur-3xl md:left-1/2 md:top-24 md:h-96 md:w-[36rem]" />
          </>
        ) : null}

        <div className="max-w-6xl mx-auto relative z-10">
          <div className={`text-center ${isCinematicHero ? 'mt-6 md:mt-10 mb-12' : 'mt-2 mb-10'}`}>
            <div className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium mb-6 animate-float ${isCinematicHero ? 'border border-white/20 bg-white/12 text-white backdrop-blur-sm' : 'border border-white/20 bg-white/10 text-white backdrop-blur-md'}`}>
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
              Premium intercity rides now live
            </div>
            <div className={`${isCinematicHero ? 'mx-auto max-w-3xl rounded-[2rem] border border-white/10 bg-slate-950/10 px-6 py-6 backdrop-blur-sm md:px-10 md:py-8' : ''}`}>
              <h1 className={`${isCinematicHero ? 'text-3xl md:text-5xl' : 'text-5xl md:text-7xl'} font-black text-white tracking-tighter leading-tight drop-shadow-lg`}>
                Travel Smart.
                <span className="block text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-blue-400">
                  Travel in Style.
                </span>
              </h1>
              <p className={`font-medium mt-6 max-w-2xl mx-auto drop-shadow-md ${isCinematicHero ? 'text-base text-slate-100 md:text-lg' : 'text-lg text-slate-300'}`}>
                Reliable, comfortable, and premium rides at your fingertips. Experience the journey, not just the destination.
              </p>
            </div>
          </div>

          <div ref={contactBarRef} className={`w-full flex flex-col items-center justify-center gap-4 relative z-10 ${isCinematicHero ? 'mb-20 sm:flex-row' : 'mb-16 sm:flex-row'}`}>
            <span className={`rounded-full py-1.5 px-4 text-sm font-semibold shadow-sm ${isCinematicHero ? 'border border-white/20 bg-white/10 text-slate-100 backdrop-blur-md' : 'border border-white/10 bg-black/30 text-slate-200 backdrop-blur-md'}`}>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              <button
                type="button"
                className={`${isCinematicHero ? 'border border-white/20 bg-emerald-500/20 backdrop-blur-md hover:bg-emerald-500/30' : 'btn-accent'} flex items-center gap-2 rounded-full px-6 py-2.5 font-semibold text-white sweep-hover transition-all duration-200`}
              >
              ...
              <button
                type="button"
                className={`${isCinematicHero ? 'border border-white/20 bg-blue-500/20 backdrop-blur-md hover:bg-blue-500/30' : 'bg-gradient-to-r from-blue-500 via-indigo-500 to-blue-600 shadow-[0_6px_18px_rgba(59,130,246,0.35)] hover:shadow-[0_8px_22px_rgba(59,130,246,0.45)]'} flex items-center gap-2 rounded-full px-6 py-2.5 font-semibold text-white sweep-hover transition-all duration-200`}
              >
